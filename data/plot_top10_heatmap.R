...
# optional args: n_genes, n_conditions
n_genes <- if(length(args) >= 3) as.integer(args[3]) else 10
...
# Wrap processing into a function so it can be reused
plot_top_genes_heatmap <- function(infile, outfile, n_genes = 10, n_conditions = 30, min_numeric_frac = 0.6){
  ...
  num_cols <- names(frac_numeric)[frac_numeric >= min_numeric_frac]
...
# Select top N genes by CV
  top_genes <- expr_filt %>% arrange(desc(cv)) %>% slice_head(n = n_genes) %>% pull(Gene)
...
# Choose top N conditions by variance (across all genes) and restrict the matrix
  condition_sd <- apply(expr[num_cols], 2, sd, na.rm = TRUE)
  condition_sd[is.na(condition_sd)] <- 0
  top_conditions <- names(sort(condition_sd, decreasing = TRUE))[1:min(n_conditions, length(condition_sd))]

  # reorder conditions by original file order but only keep selected top_conditions
  condition_levels <- intersect(num_cols, top_conditions)
  gene_levels <- top_genes
  long <- long %>% filter(Condition %in% condition_levels)
  long$Gene <- factor(long$Gene, levels = gene_levels)
  long$Condition <- factor(long$Condition, levels = condition_levels)

  cat(sprintf("Using %d conditions (top by variance)\n", length(condition_levels)))
...
p <- ggplot(long, aes(x = Gene, y = Condition, fill = Value)) +
    ...
    scale_fill_distiller(palette = "PRGn", na.value = "grey95") +
    labs(x = "Gene", y = "Condition", fill = "Expression (log)", title = "Top genes by CV") +
  ...
  cat("Top genes (by CV):\n")
  print(top_genes)
  write.csv(data.frame(Gene = top_genes), file = "top_genes.csv", row.names = FALSE)

  ggsave(outfile, p, width = 5, height = 5, units = "in", dpi = 300, bg = "white")
  cat("Saved heatmap to:", outfile, "\n")

  invisible(list(top_genes = top_genes, top_conditions = condition_levels, plot = p))
}

# Call the function with provided args
plot_top_genes_heatmap(infile = infile, outfile = outfile, n_genes = n_genes, n_conditions = n_conditions)
